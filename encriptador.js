...
document.getElementById("encriptar").addEventListener("click", function() {
    ...
    toggleOutputElements(); // Verifica encriptar
});
...
document.getElementById("desencriptar").addEventListener("click", function() {
    ...
    toggleOutputElements(); // Verifica desencriptar
});


document.getElementById("copiar").addEventListener("click", function() {
    // Texto en el área de salida
    const resultado = document.getElementById("resultado");
    resultado.select();
    resultado.setSelectionRange(0, 99999); // móviles

    // Copia el texto al portapapeles
    navigator.clipboard.writeText(resultado.value).then(() => {

        // Cambia el texto del botón cuando se ha copiado
